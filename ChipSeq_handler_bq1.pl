...
{
  $size_f = "/home/comp/twlab/twang/twlab-shared/genomes/hg19/hg19_chrom_sizes";
}
...
print "bedItemOverlapCount null chromSize=$size_f $name.extended.bed > $name.extended.bedGraph\n";
system ( "bedItemOverlapCount null chromSize=$size_f $name.extended.bed > $name.extended.bedGraph" );
...
sub bed5_2_bed9
{
        ...
        while (<IN>)
        {
                ...
                @line = split/\t/;
                $cnt ++;
                print OUT "$line[0]\t$line[1]\t$line[2]\t$line[3]\t0\t$line[4]\t0\t0\t";
                if ($line[4] eq "+")
                {
                        print OUT "255,0,0\n";
                }
