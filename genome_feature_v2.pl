...
if ($genome eq "hg19") {
	...
	$out_f = "/home/hyungjoo/genomes/danRer7_database/genome_feature.bed";
} else {
...
my @size = (0, 0, 0, );
my $cnt_skip = 0;
my @prev_line = ("chr1", "0", "0", "example", "0", "+", "0", "0,", "0,");
while (<GENE>) {
	chomp;
	my @gene_line = split;
...
#		print "$prev_line[0]\t$prev_line[1]\t$prev_line[2]\t$prev_line[3]\n$_\n" if ($size_promoter < 0);
		...
		print OUT "$prev_line[0]\t$prev_line[2]\t$promoter\tpromoter\n";
	}
...
#		print if ($size_promoter < 0);
		$size[0] += $size_promoter;
		print OUT "$gene_line[0]\t$promoter\t$gene_line[1]\tpromoter\n";
	}
	for (my $i = 0; $i < $gene_line[6]; $i++) {
		print OUT "$gene_line[0]\t$exon_start[$i]\t$intron_start[$i]\texon\n";
		print OUT "$gene_line[0]\t$intron_start[$i]\t$exon_start[$i+1]\tintron\n" if ($i != $gene_line[6]-1);
	}
	@prev_line = @gene_line;
}
close GENE;
