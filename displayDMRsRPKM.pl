...
my $usage = '
displayDMRsRPKM.pl <DMR rpkm value file> 
...
while ( <IN> )
{
  ...
  {
    # call
    if ( $status[$#status][$i] < $r1 )
    {
      $status[$#status][$i] = 0;
    }
    elsif ( ( $status[$#status][$i] >= $r1 ) && ( $status[$#status][$i] < $r2 ) )
    {
      $status[$#status][$i] = 1;
    }
    elsif ( ( $status[$#status][$i] >= $r2 ) && ( $status[$#status][$i] < $r3 ) )
    {
      $status[$#status][$i] = 2;
    }
    elsif ( ( $status[$#status][$i] >= $r3 ) && ( $status[$#status][$i] < $r4 ) )
    {
      $status[$#status][$i] = 3;
    }
    elsif ( ( $status[$#status][$i] >= $r4 ) && ( $status[$#status][$i] < $r5 ) )
    {
      $status[$#status][$i] = 4;
    }
    else
...
#@sorted = @status ;
@sorted = sort { $h_0{$a}<=>$h_0{$b} or $h_1{$a}<=>$h_1{$b} or $h_2{$a}<=>$h_2{$b} or $h_3{$a}<=>$h_3{$b} or $h_4{$a}<=>$h_4{$b} or $h_5{$a}<=>$h_5{$b} or $h_6{$a}<=>$h_6{$b} or $h_7{$a}<=>$h_7{$b} or $h_8{$a}<=>$h_8{$b} or $h_9{$a}<=>$h_9{$b} or $h_10{$a}<=>$h_10{$b} or $h_11{$a}<=>$h_11{$b}} @status ;
