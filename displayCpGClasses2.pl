...
# chr	start	end	cpg	type	dist	gene	CAGE	H3K4	MeDIP	MRE	Smart	SmartRKPM	Rna	RnaRPKM
# chr1	1579936	1580805	CpG:83	type2	77	CDC2L1	0	2.94706559263521	1.15075	8.95000
# chr1	1579936	1580805	CpG:83	type2	77	CDC2L1	0	2.94706559263521	1.15075	8.95000	38	1.1187	68	0.8501
...
while ( <IN> )
{
  ...
  if ( $line[4] =~ /$type/ )
  {
    push @status, [ ( $line[7], $line[8], $line[9], $line[10], $line[13]==0? 0 : $line[11]/$line[13] ), $line[11] ];
    ...
    # RNA
    if ( $status[$#status][4] < $rna_c || $status[$#status][5]<6 )
    {
      $status[$#status][4] = -1;
    }
    else
...
for ( my $i=0; $i<=$#status; $i++ )
{
  ...
  $ordered[$i][4] = $status[$i][ $order[4] ];
  
  $h_0{ $ordered[$i] } = $ordered[$i][0];
  $h_1{ $ordered[$i] } = $ordered[$i][1];
  $h_2{ $ordered[$i] } = $ordered[$i][2];
  $h_3{ $ordered[$i] } = $ordered[$i][3];
  $h_4{ $ordered[$i] } = $ordered[$i][4];
}

# sort
my @sorted;

@sorted = reverse ( sort { $h_0{$a}<=>$h_0{$b} or $h_1{$a}<=>$h_1{$b} or $h_2{$a}<=>$h_2{$b} or $h_3{$a}<=>$h_3{$b} or $h_4{$a}<=>$h_4{$b} } @ordered );
