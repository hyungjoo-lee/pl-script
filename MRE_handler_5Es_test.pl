...
sub build_PE_hash
{
  ...
  if( $enzyme_num == 5 )
  {
    ...
    {
    @line = split /\t/;
    if($line[3] eq "CGCG" || $line[3] eq "CGCG" )
    {
...
sub filter_reads_by_mre_sites
{
  ...
  while ( <IN> )
  {
    ...
    if ( defined( $CGCG_PE{$site} ) )   # this read falls on a CGCG site [but count for CpG would be doubled due to 2 CpG's]
    {
            $CGCG_cnt{ $site }++;
            $CGCG_reads++;
            print OUT;
    ...
    elsif ( defined( $CCGG_PE{$site} ) )   # this read falls on a CCGG site
    {
      $CCGG_cnt{ $site }++;
      $CCGG_reads++;
      print OUT;
    ...
    elsif ( defined( $ACGT_PE{$site} ) )   # this read falls on a ACGT site
    {
      $ACGT_cnt{ $site }++;
      $ACGT_reads++;
      print OUT;
...
#    elsif ( defined( $CGCG_PE{$site} ) )   # this read falls on a CGCG site [but count for CpG would be doubled due to 2 CpG's]
#    {
#	    $CGCG_cnt{ $site }++;
#            $CGCG_reads++;
#	    print OUT;
