...
<database> is hg18, hg19, mm9 or danRer7
...
print "bedSort $name.bed $name.bed\n";
system ( "bedSort $name.bed $name.bed" );
...
sub bwt_2_bed
{
  ...
  while ( <IN> )
  {
    ...
    elsif (($line[2] =~ m/(chr)\d+/) or ($line[2] eq "chrX") or ($line[2] eq "chrY") or ($line[2] eq "chrM") or ($line[2] =~ m/(Zv9_scaffold)\d+/) or ($line[2] =~ m/(Zv9_NA)\d+/))
    {
        $chr = $line[2];
    }
    else 
	...
	my $score = 0;
	print OUT "$chr\t$start\t$chrend\t$name\t$score\t+\t0\t0\t255,0,0\n";
    }
  }
  close OUT;
  close IN;
...
while (<IN>)
{
	...
	$cnt++;
        chomp;
        @line = split /\t/;
	if ($cnt % 2 == 1) {		## first read in pair
		$mapQ = $line[4];
		next;
	}
	$cnt[0]++;			## total paired reads

	$chr = $line[2];
	my $start;
	my $length = $line[8];
	next if ($length == 0);
	$cnt[1]++;			## mapped paired reads
	
	push @length, abs($length);

	next if (abs($length) > 700);	## long-distance paired reads
	$cnt[2]++;
...
printf REPORT "total paired reads:\t%10d\nmapped paired reads:\t%10d (including long reads, >700bp)\nmapped paired reads:\t%10d\nreliable paired reads(mapQ >= 10):\t%10d\n", @cnt;

print REPORT "Fragment Length Distribution\n";
print REPORT "\tFrom\tTo\tCount\n";
my $max = 0;
my @dist;
for (@length) {
	my $dist = int ($_ / 5);
...
for (my $i = 0; $i < $max/5; $i++) {
	next if !defined $dist[$i];
	my $start = $i * 5;
	my $end = $start + 4;
	printf REPORT "\t$start\t$end\t$dist[$i]\n";
}
